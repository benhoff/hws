...
capture_remap_registers() {
	...
	local out_path=$2
	local path
	local shared_hi_off shared_lo_off candidate8_hi_off candidate8_lo_off window_off
	local shared_hi shared_lo candidate8_hi candidate8_lo window

	path=$(bar0_path)
	shared_hi_off=$((0x208 + ch * 8))
	shared_lo_off=$((0x20c + ch * 8))
	candidate8_hi_off=$((0x208 + (8 + ch) * 8))
	candidate8_lo_off=$((0x20c + (8 + ch) * 8))
	window_off=$((0x4060 + ch * 4))

	shared_hi=$(bar0_read32 "$path" "$shared_hi_off")
	shared_lo=$(bar0_read32 "$path" "$shared_lo_off")
	candidate8_hi=$(bar0_read32 "$path" "$candidate8_hi_off")
	candidate8_lo=$(bar0_read32 "$path" "$candidate8_lo_off")
...

	{
		...
		printf 'candidate8_hi_offset=0x%03x value=%s\n' "$candidate8_hi_off" "$candidate8_hi"
		printf 'candidate8_lo_offset=0x%03x value=%s\n' "$candidate8_lo_off" "$candidate8_lo"
...
run_remap_validation() {
	...
	run_remap_trial "remap.trial_a_candidate8_slot" "$dev" "$ch" "$((0x208 + (8 + ch) * 8))" "$((0x20c + (8 + ch) * 8))" "$out_dir"
...
4. Intrusive BAR remap validation:
   ...
   - Inspect the passive snapshot plus both discriminator trials to determine
     whether audio depends on the shared ch slot or the candidate 8+ch bank.
